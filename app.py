...
# --- データベース設定 ---
def init_db():
    conn = sqlite3.connect('sanrentan_v16.db', check_same_thread=False)
    c = conn.cursor()
...
if mode == "参加者画面":
    ...
    # 状態が変わった場合のみ画面を強制リフレッシュするフラグ
    current_status = f"{conf['is_open']}-{conf['current_q']}"
    
    if conf['is_open'] == 1:
        st.success("🟢 現在、回答を受付中です！")
        # フォーム入力中にリロードされないよう form を使用
        with st.form("vote_form", clear_on_submit=False):
            ...
            if st.form_submit_button("予想を送信"):
                if name and "未選択" not in [g1, g2, g3] and len({g1, g2, g3}) == 3:
                    ...
                    st.error("入力内容を確認してください（重複不可）")
    else:
        st.error("🔴 現在、回答は締め切られています。")

    # 5秒待機して「問題番号」か「受付状態」が変わっていたらリロード
    time.sleep(5)
...
# 2. 投影画面
    ...
    for i, opt in enumerate(options_list):
        cols[i].markdown(f"<div style='text-align: center; background: #262730; color: white; padding: 20px; border-radius: 15px; font-size: 30px; font-weight: bold; border: 2px solid #4B4B4B;'>{opt}</div>", unsafe_allow_html=True)
    if conf['show_ans'] == 1:
        st.markdown("<h1 style='text-align: center; color: #ff4b4b; font-size: 100px;'>正解発表！</h1>", unsafe_allow_html=True)
...
# 4. 管理者
    ...
    if not st.session_state.admin_logged_in:
        ...
        st.subheader("🎯 採点")
        if conf['is_open'] == 1:
            st.warning("締め切りにしてから採点してください")
        else:
            cur_q = int(conf['current_q'])
            df_q = pd.read_sql_query(f"SELECT name, g1, g2, g3 FROM scores WHERE q_id={cur_q}", conn)
            target = st.selectbox("出題者の回答読込", ["-- 手動 --"] + list(df_q['name']))
            defs = [u_row['g1'] if target != "-- 手動 --" else "未選択" for u_row in [df_q[df_q['name'] == target].iloc[0]] if target != "-- 手動 --"] # 簡易化
            # (以下、以前の採点ロジックと同じ)
            ans1 = st.selectbox("1位正解", ["未選択"] + options_list)
            ans2 = st.selectbox("2位正解", ["未選択"] + options_list)
            ans3 = st.selectbox("3位正解", ["未選択"] + options_list)
            if st.button("採点実行"):
                ...
                conn.commit(); st.success("完了"); st.balloons()
