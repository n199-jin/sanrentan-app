...
# --- データベース設定 ---
def init_db():
    conn = sqlite3.connect('sanrentan_v12.db', check_same_thread=False)
    c = conn.cursor()
    ...
    # 初期データに q_text（問題文）を追加
    c.execute("""INSERT OR IGNORE INTO settings (id, options, is_open, current_q, last_ans1, last_ans2, last_ans3, show_ans, q_text) 
                 VALUES (1, 'A,B,C,D', 0, 1, '', '', '', 0, 'ここに問題文が表示されます')""")
    # 既存DBへのカラム追加用（エラー回避）
    try:
        c.execute("ALTER TABLE settings ADD COLUMN q_text TEXT DEFAULT '問題文'")
    except:
        pass
    conn.commit()
    return conn
        ...
        if st.form_submit_button("予想を送信"):
            if conf['is_open'] == 0:
                ...
                if len({g1, g2, g3}) < 3:
                    ...
                    conn.commit(); st.success(f"✅ 受理しました！"); st.balloons()
            else:
                st.error("⚠️ 入力不備があります。")

# --- 2. 【投影用】問題・正解表示画面 ---
elif mode == "【投影用】問題・正解表示":
    st.markdown(f"<p style='text-align: center; font-size: 40px; color: gray; margin-bottom: 0;'>第 {conf['current_q']} 問</p>", unsafe_allow_html=True)
    st.markdown(f"<h1 style='text-align: center; font-size: 70px; margin-top: 0;'>{conf['q_text']}</h1>", unsafe_allow_html=True)
    st.divider()
    
    # 選択肢の表示（デザイン修正：背景を濃く、文字を白く）
    st.markdown("<p style='text-align: center; font-size: 30px;'>【 選択肢 】</p>", unsafe_allow_html=True)
    
    # 選択肢が多すぎる場合に備えて折り返し表示
    cols = st.columns(5)
    ...
    if conf['show_ans'] == 1:
        ...
        st.markdown("<h2 style='text-align: center; color: #555;'>正解発表をお楽しみに...</h2>", unsafe_allow_html=True)
...
# --- 4. 管理者画面 ---
    ...
    if not st.session_state.admin_logged_in:
        ...
        with c_m1:
            ...
            new_q_text = st.text_input("問題文を入力", value=conf['q_text'])
        ...
        if st.button("採点実行＆投影画面に正解を表示"):
            if "未選択" not in [ans1, ans2, ans3]:
                for _, row in df_ans.iterrows():
                    sc = calculate_score([ans1, ans2, ans3], [row['g1'], row['g2'], row['g3']])
