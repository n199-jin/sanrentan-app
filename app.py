...
# --- データベース設定 ---
def init_db():
    conn = sqlite3.connect('sanrentan_v32.db', check_same_thread=False)
    c = conn.cursor()
...
# --- UI設定 ---
# initial_sidebar_state="expanded" で、起動時に必ずサイドバーを開く設定にします
st.set_page_config(page_title="サンレンタンシステム", layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
    /* ヘッダーは消すが、サイドバー開閉用のボタンは残るように設定 */
    header[data-testid="stHeader"] { visibility: visible; background: rgba(0,0,0,0); }
    ...
    /* ナビゲーション用ボタンスタイル */
    .nav-hint { font-size: 12px; color: #666; margin-bottom: 5px; }
</style>
...
with st.sidebar:
    st.title("🎮 サンレンタン")
    # session_stateと連動させることで、画面上のボタンからも切り替え可能に
    ...
    st.divider()
    st.caption("※サイドバーが閉じたら左上の「>」で開けます")

# --- 2. 画面上部の補助ナビゲーション（サイドバーが消えた時の保険） ---
# 投影用モニター以外で表示
if st.session_state.page_mode != "【投影用】メインモニター":
    with st.expander("🌐 画面切り替えメニュー（サイドバーが隠れている場合）"):
        c1, c2, c3 = st.columns(3)
        if c1.button("参加者画面へ"): st.session_state.page_mode = "参加者画面"; st.rerun()
        if c2.button("投影画面へ"): st.session_state.page_mode = "【投影用】メインモニター"; st.rerun()
        if c3.button("管理者画面へ"): st.session_state.page_mode = "管理者画面"; st.rerun()

# --- メイン描画ロジック ---
mode = st.session_state.page_mode

# 特別枠：ランキング表示（URLに ?page=rank がある場合）
...
# C. 管理者画面
    ...
    if not st.session_state.get('admin_logged_in', False):
        ...
        if st.button("反映"):
            conn.cursor().execute("UPDATE settings SET current_q=?, q_text=?, options=?, is_open=?, show_ans=0 WHERE id=1", (new_q, new_txt, new_opts, 1 if st.radio("状態", ["締め切り", "受付中"], horizontal=True)=="受付中" else 0))
            conn.commit(); st.rerun()
        # ... 以下、採点・リセットロジックを保持 ...
