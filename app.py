...
# --- 1. データベース初期設定 ---
def init_db():
    # データベース名は重複を避けるためv33としています
    conn = sqlite3.connect('sanrentan_v33.db', check_same_thread=False)
    c = conn.cursor()
...
# --- 3. 画面レイアウト設定 ---
st.set_page_config(
    page_title="サンレンタンシステム", 
    layout="wide", 
    initial_sidebar_state="expanded" # 常にサイドバーを表示した状態で開始
)

# カスタムCSS: 背景色固定とプロジェクター投影時の残像防止
st.markdown("""
<style>
...
if 'page_mode' not in st.session_state:
    st.session_state.page_mode = "参加者画面"
...
with st.sidebar:
    ...
    selected_mode = st.radio(
        "モード選択", 
        ["参加者画面", "【投影用】メインモニター", "管理者画面"],
        key="main_nav"
    )
    st.session_state.page_mode = selected_mode
    st.divider()
    st.caption("サイドバーが隠れたら左上の「>」で戻せます")

# --- 5. メインロジック ---

# 特殊モード：管理者限定ランキング（URLに?page=rankがある場合）
...
if mode == "参加者画面":
        ...
        if conf['show_ans'] == 1:
            ...
            if my_answer is not None:
                ...
                with st.form("vote_form"):
                    u_name = st.text_input("あなたの名前", value=st.session_state.my_name)
                    ...
                    if st.form_submit_button("予想を送信"):
                        ...
                        else: st.error("名前を入力し、重複のないように3名選んでください。")
        else: st.info("⌛ 次の問題を準備中です。しばらくお待ちください。")
    
    # 3秒おきに状態をチェックし、変化があれば自動更新
    time.sleep(3)
    ...
    # プロジェクター用。後ろの要素が見えないように、常に書き換えを行う。
    with st.container():
        if conf['show_ans'] == 1:
            ...
            st.markdown(f"<p style='text-align:center; color:gray; font-size:30px;'>第 {conf['current_q']} 問</p>", unsafe_allow_html=True)
            st.markdown(f"<h1 class='q-title'>{conf['q_text']}</h1>", unsafe_allow_html=True)
            st.divider()
    ...
    if not st.session_state.get('admin_logged_in', False):
        ...
        new_txt = st.text_input("問題文（例：第1レースの勝者は？）", value=conf['q_text'])
        new_opts = st.text_area("選択肢（カンマ区切り：A,B,C...）", value=conf['options'])
        status = st.radio("回答受付状態", ["締め切り", "受付中"], index=1 if conf['is_open'] == 1 else 0, horizontal=True)
        
        if st.button("設定を反映して次へ（または受付開始）"):
        ...
        st.divider()
        if conf['is_open'] == 0:
            st.subheader("🎯 正解入力と採点")
            # 出題者（模範解答）のデータを取得して自動入力の補助にする
            df_ans = pd.read_sql_query(f"SELECT name, g1, g2, g3 FROM scores WHERE q_id={conf['current_q']}", conn)
            target = st.selectbox("正解パターンの参照（出題者を選択）", ["-- 手動で選ぶ --"] + list(df_ans['name']))
            
            iv = ["未選択"]*3
            if target != "-- 手動で選ぶ --":
                row = df_ans[df_ans['name'] == target].iloc[0]
            ...
            if st.button("採点を確定して正解を表示"):
                # 模範解答として保存
                conn.cursor().execute("INSERT OR REPLACE INTO scores (q_id, name, g1, g2, g3, score) VALUES (?, '模範解答', ?, ?, ?, 0)", (int(conf['current_q']), a1, a2, a3))
                
                # スコア計算ロジック
                def calc_score(correct, guess):
                    if correct == guess: return 6 # サンレンタン
                    matched = len(set(correct) & set(guess))
                    if matched == 3: return 4 # サンレンプク
                    if correct[0] == guess[0] and correct[1] == guess[1]: return 3 # 1-2位
                    if matched == 2: return 2 # 2つ
                    if correct[0] == guess[0]: return 1 # 1位
                    return 0
                
                all_resp = pd.read_sql_query(f"SELECT name, g1, g2, g3 FROM scores WHERE q_id={conf['current_q']}", conn)
                for _, r in all_resp.iterrows():
                    sc = calc_score([a1, a2, a3], [r['g1'], r['g2'], r['g3']])
        ...
        st.divider()
        st.subheader("🚨 危険な操作")
        confirm_reset = st.checkbox("【警告】全回答データを削除し、第1問目からやり直すことに同意します。")
        if st.button("全データを完全にリセット", disabled=not confirm_reset):
